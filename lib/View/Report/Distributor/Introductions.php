...
class View_Report_Distributor_Introductions extends \View{
	...
	function setModel($model){
		...
		$this->add('View')->setElement('hr');
		$grid = $this->add('Grid');
		$grid->setModel($downline,['name','sponsor','introducer','month_self_bv','total_month_bv','status','joining','green_on']);
		$grid->addPaginator($ipp=100);
		// reload self view with form values
		if($form->isSubmitted()){
			$this->js()->reload(['search_distributor'=>$form['search_distributor'],'from_date'=>$form['from_date'],'to_date'=>$form['to_date'],'status'=>$form['status'],'based_on'=>$form['based_on']])->execute();
		}
